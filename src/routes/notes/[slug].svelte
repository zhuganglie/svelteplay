<script context="module">
  ...
  <article class="flex flex-col flex-grow">
    ...
    <div class="">
      ...
      <ul>
          <li>书名：{book}</li>
          <li><div class="flex space-x-2">作者：{#each authors as author}<span class="rounded px-2.5 py-0.5 bg-gray-700">{author}</span>{/each}</div></li>
          <li>出版社：{publisher}</li>
          <li>出版时间：{year} 年</li>
          <li>分类：{category}</li>
          </ul>
      ...
      <hr />
      <footer>
	<a href="/notes/" class="bg-gray-700 text-yellow-500 hover:text-gray-100 rounded px-2.5 py-0.5">&larr; 返回列表</a>
      </footer>
    </div>
  </article>